...
cpu_alert_type dma_transfer(dma_transfer_type *dma)
{
  ...
  if(dma->irq)
  {
	raise_interrupt(static_cast<irq_type>(IRQ_DMA0 << dma->dma_channel));
...
void init_gamepak_buffer()
{
  ...
  gamepak_memory_map = new gamepak_swap_entry_type[gamepak_ram_pages];
}
...
u8 *load_gamepak_page(u32 physical_index)
{
	...
	gamepack_file.seek(physical_index * (32 * 1024));
	gamepack_file.read((char *)swap_location, (32 * 1024));
