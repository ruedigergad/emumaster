...
static void fx_bra() { u8 v = PIPE; R15++; FETCHPIPE; R15 += SEX8(v); }
...
RAM(GSU.avReg[reg]) = (u8)SREG; \
RAM(GSU.avReg[reg]^1) = (u8)(SREG>>8); \
CLRFLAGS; R15++
...
#define FX_LDW(reg) u32 v; \
GSU.vLastRamAdr = GSU.avReg[reg]; \
v = (u32)RAM(GSU.avReg[reg]); \
v |= ((u32)RAM(GSU.avReg[reg]^1))<<8; \
R15++; DREG = v; \
TESTR14; \
CLRFLAGS
...
static void fx_plot_2bit()
{
	...
	c = (x^y)&1 ? (u8)(GSU.vColorReg>>4) : (u8)GSU.vColorReg;
    else
	c = (u8)GSU.vColorReg;
...
static void fx_rpix_4bit()
{
    ...
    DREG = 0;
    DREG |= ((u32)((a[0x00] & v) != 0)) << 0;
    DREG |= ((u32)((a[0x01] & v) != 0)) << 1;
    DREG |= ((u32)((a[0x10] & v) != 0)) << 2;
    DREG |= ((u32)((a[0x11] & v) != 0)) << 3;
    TESTR14;
}
...
static void fx_rpix_8bit()
{
    ...
    DREG |= ((u32)((a[0x10] & v) != 0)) << 2;
    DREG |= ((u32)((a[0x11] & v) != 0)) << 3;
    DREG |= ((u32)((a[0x20] & v) != 0)) << 4;
    DREG |= ((u32)((a[0x21] & v) != 0)) << 5;
    DREG |= ((u32)((a[0x30] & v) != 0)) << 6;
    DREG |= ((u32)((a[0x31] & v) != 0)) << 7;
    TESTR14;
}
...
#define FX_ADD(reg) \
s32 s = SUSEX16(SREG) + SUSEX16(GSU.avReg[reg]); \
...
#define FX_ADC(reg) \
s32 s = SUSEX16(SREG) + SUSEX16(GSU.avReg[reg]) + SEX16(GSU.vCarry); \
GSU.vCarry = s >= 0x10000; \
GSU.vOverflow = ~(SREG ^ GSU.avReg[reg]) & (GSU.avReg[reg] ^ s) & 0x8000; \
GSU.vSign = s; \
GSU.vZeroFlag = s; \
...
#define FX_ADC_I(imm) \
s32 s = SUSEX16(SREG) + imm + SUSEX16(GSU.vCarry); \
GSU.vCarry = s >= 0x10000; \
GSU.vOverflow = ~(SREG ^ imm) & (imm ^ s) & 0x8000; \
GSU.vSign = s; \
GSU.vZeroFlag = s; \
...
#define FX_SBC(reg) \
s32 s = SUSEX16(SREG) - SUSEX16(GSU.avReg[reg]) - (SUSEX16(GSU.vCarry^1)); \
GSU.vCarry = s >= 0; \
GSU.vOverflow = (SREG ^ GSU.avReg[reg]) & (SREG ^ s) & 0x8000; \
GSU.vSign = s; \
GSU.vZeroFlag = s; \
...
static void fx_sbk()
{
    RAM(GSU.vLastRamAdr) = (u8)SREG;
    RAM(GSU.vLastRamAdr^1) = (u8)(SREG>>8);
    CLRFLAGS;
    R15++;
}
...
static void fx_ror()
{
    u32 v = (USEX16(SREG)>>1) | (GSU.vCarry<<15);
    GSU.vCarry = SREG & 1;
    R15++; DREG = v;
    GSU.vSign = v;
    GSU.vZeroFlag = v;
...
#define FX_LMS(reg) \
GSU.vLastRamAdr = ((u32)PIPE) << 1; \
R15++; FETCHPIPE; R15++; \
GSU.avReg[reg] = (u32)RAM(GSU.vLastRamAdr); \
GSU.avReg[reg] |= ((u32)RAM(GSU.vLastRamAdr+1))<<8; \
CLRFLAGS;
...
GSU.vLastRamAdr = ((u32)PIPE) << 1; \
R15++; FETCHPIPE; \
RAM(GSU.vLastRamAdr) = (u8)v; \
RAM(GSU.vLastRamAdr+1) = (u8)(v>>8); \
CLRFLAGS; R15++;
...
#define FX_FROM(reg) \
if(TF(B)) { u32 v = GSU.avReg[reg]; R15++; DREG = v; \
GSU.vOverflow = (v&0x80) << 16; GSU.vSign = v; GSU.vZeroFlag = v; TESTR14; CLRFLAGS; } \
...
/* c1-cf - or rn */
#define FX_OR(reg) \
u32 v = SREG | GSU.avReg[reg]; R15++; DREG = v; \
GSU.vSign = v; \
GSU.vZeroFlag = v; \
TESTR14; \
CLRFLAGS;
...
static u32 fx_run(u32 nInstructions)
{
...
static u32 fx_run_to_breakpoint(u32 nInstructions)
{
    u32 vCounter = 0;
...
static u32 fx_step_over(u32 nInstructions)
{
    u32 vCounter = 0;
    while(TF(G) && vCounter < nInstructions)
    {
...
u32 (*FX_FUNCTION_TABLE[])(u32) =
#else
u32 (*fx_apfFunctionTable[])(u32) =
#endif
{
    &fx_run,
