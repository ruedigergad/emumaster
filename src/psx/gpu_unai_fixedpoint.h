...
#ifdef __arm__
static inline u32 Log2(u32 x) { u32 res; asm("clz %0,%1" : "=r" (res) : "r" (x)); return 32-res; }
#else
static inline u32 Log2(u32 x) { u32 i = 0; for ( ; x > 0; ++i, x >>= 1); return i - 1; }
#endif

#ifdef GPU_TABLE_10_BITS
static inline  void  xInv (const fixed _b, s32& iFactor_, s32& iShift_)
{
    u32 uD   = (_b<0) ? -_b : _b ;
    u32 uLog = Log2(uD);
...
static inline  fixed xInvMulx  (const fixed _a, const s32 _iFact, const s32 _iShift)
{
	#ifdef __arm__
		s64 res;
...
static inline  fixed xLoDivx   (const fixed _a, const fixed _b)
{
  s32 iFact, iShift;
...
///////////////////////////////////////////////////////////////////////////
template<typename T>
static inline  T Min2 (const T _a, const T _b)             { return (_a<_b)?_a:_b; }

template<typename T>
static inline  T Min3 (const T _a, const T _b, const T _c) { return  Min2(Min2(_a,_b),_c); }

///////////////////////////////////////////////////////////////////////////
template<typename T>
static inline  T Max2 (const T _a, const T _b)             { return  (_a>_b)?_a:_b; }

template<typename T>
static inline  T Max3 (const T _a, const T _b, const T _c) { return  Max2(Max2(_a,_b),_c); }
