...
//	SysPrintf("delayRead at %x!\n", psxRegs.pc);
	...
	psxCpuIntExec(); // first branch opcode
...
static void psxTestSWInts() {
	...
	if (psxRegs.CP0.n.Cause & psxRegs.CP0.n.Status & 0x0300 &&
		...
		psxCpuException(psxRegs.CP0.n.Cause, branch);
	}
}
...
static void cpuIntExecute() {
	for (; !stop; )
		psxCpuIntExec();
}

static void cpuIntExecuteBlock() {
	branch2 = 0;
	while (!branch2)
		psxCpuIntExec();
}

static void cpuIntClear(u32, u32) {
}
...
bool PsxCpuInt::init() {
	...
	execute			= cpuIntExecute;
	executeBlock	= cpuIntExecuteBlock;
	clear			= cpuIntClear;
