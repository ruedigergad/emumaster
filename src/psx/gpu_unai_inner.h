...
#include "gpu_unai_inner_blend.h"
#include "gpu_unai_inner_light.h"

///////////////////////////////////////////////////////////////////////////////
//  GPU Pixel opperations generator
template<const int CF>
static inline void gpuPixelFn(u16 *pixel,const u16 data)
{
	if ((!M)&&(!B))
	{
...
template<const int CF>
static inline void  gpuTileSpanFn(u16 *pDst, u32 count, u16 data)
{
	if ((!M)&&(!B))
	{
...
template<const int CF>
static inline void  gpuSpriteSpanFn(u16 *pDst, u32 count, u32 u0, const u32 mask)
{
	u16 uSrc;
	u16 uDst;
...
template<const int CF>
static inline void  gpuPolySpanFn(u16 *pDst, u32 count)
{
	if (!TM)
	{	
		// NO TEXTURE
